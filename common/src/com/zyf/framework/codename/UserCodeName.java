...
/**
 * 用于业务实体引用{@link User}作为属性, 在保存业务实体时把用户登录名<code>username</code>
 * 写到实体相关的数据库表, 检索业务实体时把保存在业务实体相关的数据库表中的用户名转换成实际
 * 的{@link User}, 在实际应用中绝大多数情况下要使用这类组件, 而不是显式地建立引用, 特别是指
 * 使用其他系统维护的实体, 而且仅仅是读取这个相关实体的属性的情况
 * @author zhangli
 * @version $Id: UserCodeName.java,v 1.1 2007/11/05 03:16:00 yushn Exp $
 * @since 2007-4-8
 */
public class UserCodeName extends User implements Principal {
    ...
    /**
     * 这个用户是否拥有参数指定的角色
     * @param roleCode 角色编码, 注意这个参数可以是","分隔的角色编码列表
     * @return 如果拥有参数指定的角色返回<code>true</code>
     * @throws NullPointerException 如果参数是<code>null</code>,<code>empty string</code>
     */
    public boolean hasRole(String roleCode) {
        AssertUtils.notNull(roleCode);
        Set roleCodes = org.springframework.util.StringUtils.commaDelimitedListToSet(roleCode);
        boolean ret = false;
