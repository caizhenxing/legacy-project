...
/**
 * <class>TransferImportManager</class> 提供了导入数据时需要的方法, 实现类应该是 {@link com.zyf.framework.transfer.core.support.TransferImportDaoSupport} 的子类, 
 * 其中的 {@link com.zyf.framework.transfer.core.TransferImportTemplate} 对 {@link TransferImportManager} 中的方法提供了绝大多数支持, 通常情况下你只需要简单的实现对
 * 欲导入的表进行增删改操作即可
...
public interface TransferImportManager {
	 ...
	 * 
	 * <p>
	 * 实现类只需要调用 {@link com.zyf.framework.transfer.core.TransferImportOperations#importData(InputStream, FileType, Date, TransferImportCallback)}, 
	 * 再简单的实现新增数据功能即可
	 * 
	 ...
	 *  
	 * <p>
	 * 实现类只需要调用 {@link com.zyf.framework.transfer.core.TransferImportOperations#processDataAfterImported(Serializable[], TransferImportProcessDataCallback)}, 
	 * 再简单的实现删除数据功能即可
	 *   
