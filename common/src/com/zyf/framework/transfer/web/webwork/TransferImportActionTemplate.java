...
import com.zyf.framework.transfer.manager.TransferImportManager;
 ...
 * @since 2005-9-26
 * @author 王政
 * @version $Id: TransferImportActionTemplate.java,v 1.1 2007/12/08 08:17:12 lanxg Exp $
 */
public abstract class TransferImportActionTemplate extends ActionSupport implements Preparable {
	...
	public static final String IMPORT_RESULT_SESSION_KEY = "com.zyf.framework.transfer.web.webwork.TransferImportActionTemplate.importResult";
	...
	/** 导入日期, 有些模块不需要使用 */
	protected Date importDate = new Date();
	...
	/**
	 * 得到导入数据的属性元信息
	 * @see com.zyf.framework.transfer.meta.ImportMetaData#getEntityMap()
	...
	private boolean validateBeforeImport() {
		...
		if (requireImportDate() && getImportDate() != null && getImportDate().after(new Date())) {
