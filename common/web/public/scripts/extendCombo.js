 ...
 * @author pillarliu
 * @version $Id: extendCombo.js,v 1.2 2008/01/18 07:43:32 hepan Exp $
 *
...
ExtendCombo.setMenu = function (temp_obj,optionInfos,nextPage, temp_btn,temp_hidd,temp_count,multStr,isClear,fun){
	...
	if(pageCount > 1){
		...
		for(var i = beginPage; i < maxPage; i++ ){
			if(i == nextPage)
				tabString+="&nbsp;<a href='#' onclick='parent.ExtendCombo.setMenu(parent.ExtendCombo.txtObj,parent.ExtendCombo.tempList," + i + ", parent.ExtendCombo.btnObj,parent.ExtendCombo.hidObj,"+arg5+",\""+arg6+"\")'><font color=red>" + (i + 1) + "</font></a>";
			else
				tabString+="&nbsp;<a href='#' onclick='parent.ExtendCombo.setMenu(parent.ExtendCombo.txtObj,parent.ExtendCombo.tempList," + i + ", parent.ExtendCombo.btnObj,parent.ExtendCombo.hidObj,"+arg5+",\""+arg6+"\")'>" + (i + 1) + "</a>";
		}				
...
ExtendCombo.selectOptions = function(systemModuleName, hiddenObject, textObject, buttonObject, type) {
	ExtendCombo.hostUrl = ContextInfo.contextPath + '/common/systemcode.do?step=comboSupportList&systemModuleName=' + systemModuleName + '&timpstamp=' + new Date().getTime() + '&parentCode=';
 ...
 */
ExtendCombo.getOptionStatic = function (txtObj,btnObj,hiddObj,arr,fun,isCountPage){
	if(arr == '')
		return;
	arr = ExtendCombo.query(arr,txtObj,btnObj);
	//是否使用分页，10为分页，11为不分页
	isCountPage = arr.length > 30 ?  10 : 11 ;
	ExtendCombo.setMenu(txtObj, arr,0,btnObj,hiddObj,isCountPage,'','',fun);
}
