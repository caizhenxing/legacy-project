...
if(typeof displaySelectLayer != 'object'){
	document.writeln('<iframe id=displaySelectLayer Author=heerit frameborder=0 style="border:0px;position: absolute; width: 160px; height: 0px; z-index: 9998; display: none;" scrolling=yes></iframe>');
...
function displaySelectMenu(tabString,temp_obj,temp_btn,local_page_width,local_page_height,isPageCount,flag){
	...
	var thei  = temp_obj.clientHeight+1;  //TT控件本身的高
	...
	var pageMainColor  = "#eeeeee";
	var pageLineColor  = "#aaaaaa";
	...
	if(!global_explorer_ie6 || flag){
		var win   = window.frames.displaySelectLayer;
		...
		divs.width = "160";
		if(isPageCount){
			var twidth  = win.document.all.tableId_selectList.clientWidth+2
			local_page_width  = (twidth > local_page_width)?twidth : local_page_width ;			
		}
		divs.width = local_page_width;
		if(isPageCount){
			var tempHeight = win.document.all.tableId_selectList.offsetHeight+2;
		...
		SelectInputWin.show(0,thei,local_page_width, local_page_height,temp_obj);
		var tabContext = SelectInputWin.document.getElementById("tableId_selectList");
		var twidth  = tabContext.clientWidth+0
		var theight = tabContext.clientHeight+0
		local_page_width  = (twidth > local_page_width)?twidth : local_page_width ;			
		if(isPageCount){	
			SelectInputWin.show(0,thei, local_page_width, theight,temp_obj);
		}
	}
}
...
function setSelectMenu(temp_obj,optionInfos,nextPage, temp_btn){
	...
	var tabTableString  = '';
	var tabSelectString = '';
	var tabHtmlString   = '<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="zh-CN"><head><link href="'+getContextPath()+'/css/style.css" rel="stylesheet" type="text/css">';
	...
	tabSelectString += "	<td style=\"cursor:hand;\" title=\"将输入框置空\" class='article_copyright'>";//<img src='"+getContextPath()+"image/shenhe_wei1.gif' align='absmiddle' border=0> <img src='"+getContextPath()+"image/shenhe_no.gif' align='absmiddle' border=0>
	if(selBox)
	...
	if(pageCount > 1){
		...
		tabString+="<img src='"+getContextPath()+"/image/main/scroll_left.gif' onclick='parent.setSelectMenu(parent.global_object_input,parent.global_selectListData," + (nextPage - 2*showNum ) + ", parent.global_object_button,parent.global_object_hidden,"+arg5+",\""+arg6+"\");' style=\"cursor:hand;\" align=\"absmiddle\">";
...
function getArrayMap(){
	...
	arrayMapping.add(new codeMapping('xl',   10013));//;学历
  ...
  with(window.event){  
	 if (typeof global_date_input == 'object'){
  	 ...
  	 if (typeof global_object_input == 'object'){
	 	 if(srcElement !=global_object_input && srcElement !=global_object_button  && !global_explorer_ie6 &&displaySelectLayer )
 ...
 * 显示**级联
 * 
 * @param txtname		文本输入框名字/ID, 内容为**或专业的名称/或者对象
...
function getOptionsMajor(txtname,hidname,btnobj,fathid,granid,islink,jlname,ggranid,fathname,fathjlname,checkbox,jlyear){
	...
	//对父节点根据fathname初始化fathid
	if(fathjlname && fathname && fathname.value != '' && fatobj.value == '' ){
	...
	for(var i=0;i<string_arr.length;i++){
		...
		//只有多个父亲时候以";"区分判断
		if(jlname && node_arr[2] && node_arr[2].indexOf(";") > 0 &&fatobj.value!='' && node_arr[2].indexOf(fatobj.value)!=-1){
			fath_array[flen++] = str;
	...
	if(fhidd.value == '' || fhidd.value != faid){
		...
		*/
		...
		for(var i=0;i<data_arr.length;i++){
			...
			if(faid.indexOf(';')>=0){			//--poss: 一个专业可能对应多个**
