...
function checkAll(checkboxAll, checkbox){
	...
	if(checkboxAll.checked)
		checkboxAll.className = "box_checked";
	else
		checkboxAll.className = "";
	if('checkbox' == checkbox.type){
		checkbox.checked = checkboxAll.checked;
		if(checkbox.checked)
				checkbox.className = "box_checked";
			else
				checkbox.className = "";
	}else{	
		for(var i = 0; i < checkbox.length; i++){
			...
			if(checkbox[i].checked)
				checkbox[i].className = "box_checked";
			else
				checkbox[i].className = "";
		}
	}
}
...
function openWindowInFrame(url, winName,wwidth, wheight)
{
	...
	wwidth = wwidth ? wwidth : window.screen.width /2
	wheight= wheight?wheight : window.screen.height/2	
	var frameUrl =  getContextPath() + '/application/pages/select_frame.htm?url=' + escape(url);	
...
function openDialogInFrame(url, wwidth, wheight)
{
	...
	wwidth = wwidth ? wwidth : window.screen.width /2
	wheight= wheight?wheight : window.screen.height/2	
	var frameUrl =  getContextPath() + '/application/pages/select_dialog.htm';
	var returnValue = showModalDialog(frameUrl, new Array(window,url), 'dialogWidth:' + wwidth +'px;dialogHeight:' + wheight + 'px;status:no;scroll:auto;help:no;')
	return returnValue
}
...
//设置高度
function setHeight(){
	if(parent.document.getElementById("ifrm_content")){
		parent.document.getElementById("ifrm_content").style.height = document.body.scrollHeight+"px";
	}
}

function getAction(tar,url,formObj){
	if(formObj){
		formObj.target = tar;
		formObj.method = "post";
		formObj.action = url;
