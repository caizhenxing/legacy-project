...
function initializeMenu(menuId, actuatorId) {
    ...
    actuator.parentNode.style.backgroundImage = "url(<ww:url value = "'/images/plus.gif'"  />)";
    actuator.onclick = function() {
        var display = menu.style.display;
        this.parentNode.style.backgroundImage =
            (display == "block") ? "url(<ww:url value = "'/images/plus.gif'" />)" : "url(<ww:url value = "'/images/minus.gif'" />)";
...
function initializeMenus() {
    ...
    // alert('actuators.length' + actuators.length);
    
    for (i=0; i < actuators.length; i++) {
        initializeMenu(menus[i].id, actuators[i].id);
    }
	
	// alert(' initializeMenu end ');
...
function openMenu(menuId) {
    ...
    if (menu != null) {
        var display = menu.style.display;
        menu.parentNode.style.backgroundImage = "url(<ww:url value = "'/images/minus.gif'" />)";
        menu.style.display = (display == "block") ? "none" : "block";
    }
}
