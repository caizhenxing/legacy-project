...
public class AssignReaderTest extends AbstractReaderTestCase {
  ...
  public void testFromExpression() throws Exception {
    ...
    Assign.Copy copy = (Assign.Copy) assign.getCopies().get(0);
  ...
  public void testFromVariableQuery() throws Exception {
    String xml = 
      "<assign xmlns:a='urn:a'>" +
      " <copy xmlns:c='urn:c'>" +
      "  <from xmlns:f='urn:f' variable='iv' part='p' query='/p/q' />" +
      "  <to>$v/p</to>" +
      " </copy>" +
      "</assign>";
    initMessageProperties();
    ...
    assertEquals("/p/q", query.getText());
  ...
  public void testFromVariableProperty() throws Exception {
    String xml = 
      "<assign xmlns:tns='http://manufacturing.org/wsdl/purchase'>" +
      " <copy>" +
  ...
  public void testToVariableQuery() throws Exception {
    String xml = 
      ...
      " <copy xmlns:c='urn:c'>" +
      "  <from>$v/p</from>" +
      "  <to xmlns:f='urn:f' variable='iv' part='p' query='/p/q' />" +
      " </copy>" +
      "</assign>";
    initMessageProperties();
    Assign assign = (Assign) readActivity(xml);
