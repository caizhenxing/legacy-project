...
/**
 * @author Juan Cantú
 * @version $Revision: 1.13 $ $Date: 2006/08/22 04:13:10 $
 */
public class WsdlConverterTest extends TestCase {
  ...
  public void testMultipleNamespacePrefixes() throws Exception {
    String xml = 
      "<definitions xmlns='" + Constants.NS_URI_WSDL + "'" +
    ...
    // properties
    Iterator propertyIt = XmlUtil.getElements(definitions, BpelConstants.NS_BPWS, WsdlConstants.ELEM_PROPERTY);
    QName rodentName = new QName("http://rodents.net", "rodentName");
    // mouseName
    Element property = (Element) propertyIt.next();
    QName name = XmlUtil.getQName(property.getAttribute("type"), property);
    assertEquals(rodentName, name);
    // ratName
    property = (Element) propertyIt.next();
    name = XmlUtil.getQName(property.getAttribute("type"), property);
    assertEquals(rodentName, name);
  }
  ...
  public void testPropertyAliasNoQuery() throws Exception {
    String xml = "<bpws:propertyAlias messageType='tns:POMessage' propertyName='tns:invoiceId' part='PO'/>";
    Element propertyAlias = transformWrap(xml);
    // property name
    QName propertyName = XmlUtil.getQName(propertyAlias.getAttribute("propertyName"), propertyAlias);
    assertEquals("invoiceId", propertyName.getLocalPart());
    assertEquals(TNS_URI, propertyName.getNamespaceURI());
    // message type
    QName messageType = XmlUtil.getQName(propertyAlias.getAttribute("messageType"), propertyAlias);
    ...
    // no query
    assertNull(XmlUtil.getElement(propertyAlias, BpelConstants.NS_BPWS, "query"));
  }    
  
  public void testPropertyAliasQuery() throws Exception {
    String xml = 
      "<bpws:propertyAlias propertyName='tns:invoiceId' messageType='tns:POMessage' part='PO' " +
      " query='/PO/tns:orderNumber' xmlns:tns='http://www.mecachi.com'/>";
    Element propertyAlias = transformWrap(xml);
    // no query attribute
    assertFalse(propertyAlias.hasAttribute("query"));
    // query subelement
    Element query = XmlUtil.getElement(propertyAlias, BpelConstants.NS_BPWS, "query");
    assertEquals("/PO/tns:orderNumber", XmlUtil.getStringValue(query));
    // namespaces
    assertEquals(BpelConstants.NS_BPWS_1_1, DOMUtils.getNamespaceURIFromPrefix(query, "bpws"));
  ...
  public void testWriteConvertedDocument() throws Exception {
    ...
    Definition definition = wsdlFactory.newWSDLReader().readWSDL(documentURI, definitionsElem.getOwnerDocument());
    try {
  ...
  public static Element transform(Source source) throws TransformerException, SAXException {
    ...
    ImportWsdlLocator.getWsdlUpdateTemplates().newTransformer().transform(source, new StreamResult(sink));
