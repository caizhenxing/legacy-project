...
public abstract class AbstractListenerTestCase extends AbstractDbTestCase {
  ...
  private static final String WSDL_TEXT = "<definitions targetNamespace='http://jbpm.org/wsdl'"
      ...
      + "  </bpws:propertyAlias>"
      + "</definitions>";
  private static final String BPEL_TEXT = "<process name='testProcess' xmlns:def='http://jbpm.org/wsdl' xmlns:vendor='http://jbpm.org'"
  ...
  public void setUp() throws Exception {
    ...
    // read bpel
    BpelReader.getInstance().read(process,
        new InputSource(new StringReader(BPEL_TEXT)));
    ...
    event.addAction(new Action(
        new Delegation(ReceptionVerifier.class.getName())));
  ...
  protected void sendRequest() throws Exception {
    ...
    Destination destination = integrationControl.getPartnerLinkEntry(
        partnerLink).getDestination();
    Session session = null;
    try {
      // create session and producer
      session = integrationControl.getJmsConnection().createSession(false,
      ...
      // element part
      Element elementValue = XmlUtil.parseElement("<sns:surpriseElement xmlns:sns='http://jbpm.org/xsd'>"
          + " <b on=\"true\">true</b>"
          + " <c name=\"venus\"/>"
          + " <d amount=\"20\"/>" + " <e>30</e>" + "</sns:surpriseElement>");
      ...
      // set properties
      message.setLongProperty(IntegrationConstants.PARTNER_LINK_ID_PROP,
          partnerLink.getId());
  ...
  public static class ReceptionVerifier implements ActionHandler {
    ...
    public void execute(ExecutionContext exeContext) throws Exception {
      ...
      // id cset
      CorrelationSetDefinition set = correlations.getCorrelation("csId")
          .getSet();
