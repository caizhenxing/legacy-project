 ...
 * @version $Revision: 1.10 $ $Date: 2006/08/29 23:02:43 $
 */
public class AppDescriptorReader {
    ...
    try {
      ...
      if (!problemAdapter.hasErrors()) {
        ...
        Element catalogsElem = XmlUtil.getElement(descriptorElem,
            BpelConstants.NS_VENDOR, BpelConstants.ELEM_SERVICE_CATALOGS);
  ...
  public void readScope(Element scopeElem, AppScope scope) {
    ...
    Element partnerLinksElem = XmlUtil.getElement(scopeElem,
        BpelConstants.NS_VENDOR, BpelConstants.ELEM_PARTNER_LINKS);
    if (partnerLinksElem != null) readPartnerLinks(partnerLinksElem, scope);
    // subordinated scopes
    Element scopesElem = XmlUtil.getElement(scopeElem, BpelConstants.NS_VENDOR,
  ...
  public void readScopes(Element scopesElem, AppScope parentScope) {
    Iterator scopeElemIt = XmlUtil.getElements(scopesElem,
        BpelConstants.NS_VENDOR, BpelConstants.ELEM_SCOPE);
  ...
  public void readPartnerLinks(Element plinksElem, AppScope scope) {
    Iterator plinkElemIt = XmlUtil.getElements(plinksElem,
        BpelConstants.NS_VENDOR, BpelConstants.ELEM_PARTNER_LINK);
  ...
  public AppPartnerLink readPartnerLink(Element partnerLinkElem) {
    ...
    Element myRoleElem = XmlUtil.getElement(partnerLinkElem,
        BpelConstants.NS_VENDOR, BpelConstants.ELEM_MY_ROLE);
    if (myRoleElem != null) {
      partnerLink.setMyRole(readMyRole(myRoleElem));
    }
    // partner role
    Element partnerRoleElem = XmlUtil.getElement(partnerLinkElem,
        BpelConstants.NS_VENDOR, BpelConstants.ELEM_PARTNER_ROLE);
  ...
  public AppMyRole readMyRole(Element myRoleElem) {
    ...
    // partner link handle
    myRole.setHandle(XmlUtil.getAttribute(myRoleElem, BpelConstants.ATTR_HANDLE));
    return myRole;
  }
  ...
  public AppPartnerRole readPartnerRole(Element partnerRoleElem) {
    ...
    if (referenceElem != null) {
      if (InitiateMode.STATIC.equals(initiateMode)) {
        ...
        getProblemHandler().add(
            new ParseProblem("not treating element as endpoint reference, "
                + "since initiate mode is not static", referenceElem));
      }
    }
    else if (InitiateMode.STATIC.equals(initiateMode)) {
      getProblemHandler().add(
          new ParseProblem("endpoint reference missing", partnerRoleElem));
    }
    return partnerRole;
  }
  ...
  public ServiceCatalog readServiceCatalogs(Element catalogsElem,
    ...
    while (catalogElemIt.hasNext()) {
      ...
      if (catalogReader != null) {
        ...
        getProblemHandler().add(
            new ParseProblem("unrecognized service catalog", catalogElem));
      }
    }
