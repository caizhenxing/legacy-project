...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:exslt="http://exslt.org/common"
  ...
  <xsl:template match="bpel11:otherwise">
  ...
  <xsl:template
    match="bpel11:*[local-name() = 'receive' or local-name() = 'reply' or local-name() = 'invoke' or 
    local-name() = 'assign' or local-name() = 'empty' or local-name() = 'throw' or local-name() = 'compensate' or
    local-name() = 'sequence' or local-name() = 'pick' or local-name() = 'flow']">
  ...
  <xsl:template match="bpel11:scope">
    <xsl:element name="scope" namespace="{$new-uri}">
      ...
      <xsl:call-template name="copyNonBpelNamespaces" />
      <!-- attributes -->
      <xsl:copy-of select="@*[name() != 'joinCondition' and name() != 'variableAccessSerializable']" />
  ...
  <xsl:template match="bpel11:from">
    <xsl:choose>
      ...
      <!-- variable/part/query, variable/property, partnerLink -->
      <xsl:when test="@variable or @partnerLink">
