...
/**
 * @author Juan Cantú
 * @version $Revision: 1.16 $ $Date: 2006/09/11 09:32:36 $
 */
  ...
  public Replier readReplier(Element replierElem, CompositeActivity parent) {
    ...
    if (partnerLink == null) {
      bpelReader.getProblemHandler().add(
          new ParseProblem("partner link not found", replierElem));
      return replier;
    }
    ...
    if (!OperationType.REQUEST_RESPONSE.equals(operation.getStyle())) {
      bpelReader.getProblemHandler().add(
          new ParseProblem("operation is one-way", replierElem));
      return replier;
    }
    ...
    if (faultPrefixedName != null) {
      ...
      if (fault == null) {
        bpelReader.getProblemHandler().add(
            new ParseProblem("fault not found", replierElem));
        return replier;
      }
    ...
    // variable
    VariableDefinition variable = bpelReader.getMessageActivityVariable(
        replierElem, BpelConstants.ATTR_VARIABLE, parent, outgoingMessage);
