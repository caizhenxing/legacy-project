...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="bpel-old-uri" select="'http://schemas.xmlsoap.org/ws/2003/03/business-process/'" />
  <xsl:variable name="bpel-new-uri" select="'http://schemas.xmlsoap.org/ws/2004/03/business-process/'" />
  ...
  <!--propertyAlias-->
  <xsl:template match="bpel11:propertyAlias">
    <xsl:element name="bpel2:propertyAlias" namespace="{$bpel-new-uri}">
      <!-- namespaces -->
      <xsl:call-template name="copyNamespaces" />
      <!-- attributes -->
      ...
      <xsl:if test="@query">
        ...
        <xsl:element name="query" namespace="{$bpel-new-uri}">
  ...
  <xsl:template match="bpel11:*">
    <xsl:element name="bpel2:{local-name()}" namespace="{$bpel-new-uri}">
      <!-- namespaces -->
      <xsl:call-template name="copyNamespaces" />
      <!-- attributes -->    
      <xsl:copy-of select="@*" />
  ...
  <!--copies namespace declarations excluding the default, the old bpel and plt namespaces -->
  <xsl:template name="copyNamespaces">
    <xsl:for-each select="namespace::*">
      ...
      <!-- we can safely omit declarations of the old bpel and partner link namespaces,
         - as the presence of extension elements will cause the transformer to declare them -->
      <xsl:if test="name(.) and $namespace != $bpel-old-uri and $namespace != $plt-old-uri">
        <xsl:copy/>
      </xsl:if>
    </xsl:for-each>
  </xsl:template>
