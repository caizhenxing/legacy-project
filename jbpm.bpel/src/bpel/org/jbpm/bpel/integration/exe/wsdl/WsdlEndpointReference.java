...
public class WsdlEndpointReference extends SoapEndpointReference {
  ...
  protected Element writeEndpointRef(Document nodeFactory) {
    ...
    XmlUtil.addNamespaceDeclaration(serviceElem,
        Constants.NS_URI_WSDL,
        PREFIX_WSDL);
    // service name
    ...
    if (portName != null) {
      ...
      if (address != null) {
        Element addressElem = nodeFactory.createElementNS(SOAPConstants.NS_URI_SOAP,
            PREFIX_SOAP + ':' + SOAPConstants.ELEM_ADDRESS);
        XmlUtil.addNamespaceDeclaration(addressElem,
            SOAPConstants.NS_URI_SOAP,
            PREFIX_SOAP);
