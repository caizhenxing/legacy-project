...
/**
 * Translates between <code>&lt;partnerLinkType/&gt;</code> XML elements and
 * {@link PartnerLinkType} instances.
 * @author Alejandro Guízar
 * @version $Revision: 1.11 $ $Date: 2006/08/22 04:13:11 $
 */
public class PartnerLinkTypeSerializer implements ExtensionDeserializer,
  ...
  public ExtensibilityElement unmarshall(Class parentType, QName elementType,
    ...
    Iterator roleElemIt = XmlUtil.getElements(elem,
        WsdlConstants.NS_PLNK,
        WsdlConstants.ELEM_ROLE);
    ...
    if (roleElemIt.hasNext()) {
      ...
      partnerLinkType.setSecondRole(unmarshallRole(roleElem,
          partnerLinkType,
          def));
    }
  ...
  /**
   * Serializes a {@link PartnerLinkType} instance into the given
   * {@link PrintWriter}.
   * @param parentType class object indicating where in the WSDL document this
   *          extensibility element was encountered
   * @param elementType the qname of the extensibility element
  ...
  public void marshall(Class parentType, QName elementType,
    ...
    String tagName = DOMUtils.getQualifiedValue(WsdlConstants.NS_PLNK,
        WsdlConstants.ELEM_PARTNER_LINK_TYPE,
        def);
    pw.print("  <" + tagName);

    // name attribute
    DOMUtils.printAttribute(WsdlConstants.ATTR_NAME, partnerLinkType.getQName()
        .getLocalPart(), pw);
  ...
  /**
   * Deserializes a DOM element into a
   * {@link org.jbpm.bpel.wsdl.PartnerLinkType.Role} instance.
   ...
   * @return the deserialized instance
   */
  protected PartnerLinkType.Role unmarshallRole(Element roleElem,
      PartnerLinkType partnerLinkType, Definition def) {
    ...
    // portType attribute
    String portTypeName = roleElem.getAttribute(WsdlConstants.ATTR_PORT_TYPE);
    QName portTypeQName = XmlUtil.getQName(portTypeName, roleElem);
  ...
  /**
   * Serializes a {@link org.jbpm.bpel.wsdl.PartnerLinkType.Role} instance into
   ...
   * @throws WSDLException if serialization fails
   */
  protected void marshallRole(PartnerLinkType.Role role, PrintWriter pw,
      Definition def) throws WSDLException {
    if (role == null) return;

    // Open tag
    String roleTag = DOMUtils.getQualifiedValue(WsdlConstants.NS_PLNK,
        WsdlConstants.ELEM_ROLE,
        def);
    pw.print("    <" + roleTag);
