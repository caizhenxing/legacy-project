...
public class AddSaveAction extends BaseAction {
	...
	public ActionForward doMainProcessing(
		...
		ISystemServise servise = getSystemServise(
			...
			ShozokuInfo result = servise.insert(container.getUserInfo(),addInfo);
		
		if(log.isDebugEnabled()){
			log.debug("所属機関情報　登録情報 '"+ result);
		}
		
		result.setShozokuName(addInfo.getShozokuName());	//所属機関名（和文）
		
		//-----セッションに所属機関情報を登録する。
		container.setShozokuInfo(result);
