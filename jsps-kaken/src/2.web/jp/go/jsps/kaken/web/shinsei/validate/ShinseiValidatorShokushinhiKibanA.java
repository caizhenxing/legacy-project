...
/**
 * 形式チェッククラス（特別研究促進費（基盤研究(A)相当））
 * ID RCSfile="$RCSfile: ShinseiValidatorShokushinhiKibanA.java,v $"
 * Revision="$Revision: 1.3 $"
 * Date="$Date: 2007/07/13 07:18:05 $"
 */
public class ShinseiValidatorShokushinhiKibanA extends DefaultShinseiValidator {
	...
	/** 研究経費合計最低金額(円) */
	private int MIN_KENKYUKEIHI_GOKEI = 20000;

	/** 研究経費合計最高金額(円) */
	private int MAX_KENKYUKEIHI_GOKEI = 50000;
	 ...
	 * @param page
	 */
	public ShinseiValidatorShokushinhiKibanA(ShinseiDataInfo shinseiDataInfo) {
		super(shinseiDataInfo);
	}

	/**
	 * 形式チェック（特別研究促進費（基盤研究(A)相当））
	 * 
	...
	private void countKenkyushaNinzu(ActionErrors errors, int page) {
		...
		if (page >= 2) {
			...
			// 分担金の有無と研究人数を比較 ※有の場合には研究人数が2以上 ★研究分担者が存在しない場合にチェックをすり抜けていた
