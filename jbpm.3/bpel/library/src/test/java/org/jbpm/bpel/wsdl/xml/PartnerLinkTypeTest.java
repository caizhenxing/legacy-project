...
public class PartnerLinkTypeTest extends TestCase {
  ...
  public void testGetPartnerLinkType() {
    ...
    PartnerLinkType plinkType = WsdlUtil.getPartnerLinkType(definition, new QName(tns, "purchasingLT"));
    assertNotNull(plinkType);
    assertEquals("purchaseService", plinkType.getFirstRole().getName());

    plinkType = WsdlUtil.getPartnerLinkType(definition, new QName(tns, "shippingLT"));
    assertNotNull(plinkType);
  ...
  public void testUnmarshall() {
    ...
    assertEquals(new QName(NS_POS, "purchaseOrderPT"), role.getPortType().getQName());
    // Second role
    assertNull(plinkType.getSecondRole());

    // invoicing partner link
    plinkType = WsdlUtil.getPartnerLinkType(definition, new QName(tns, "invoicingLT"));
    assertNotNull(plinkType);
    // First role
    role = plinkType.getFirstRole();
    assertEquals("invoiceService", role.getName());
    assertEquals(new QName(NS_POS, "computePricePT"), role.getPortType().getQName());
    // Second role
    role = plinkType.getSecondRole();
    assertEquals("invoiceRequester", role.getName());
    assertEquals(new QName(NS_POS, "invoiceCallbackPT"),
