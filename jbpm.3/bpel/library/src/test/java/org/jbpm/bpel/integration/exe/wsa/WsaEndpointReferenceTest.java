...
import org.jbpm.bpel.integration.exe.SoapEndpointReferenceTest;
 ...
 */
public class WsaEndpointReferenceTest extends SoapEndpointReferenceTest {

  public WsaEndpointReferenceTest(String name) {
    super(name);
  }
  
  protected SoapEndpointReference getReference() {
  ...
  public void testReadEndpointRef() throws Exception {
    String text = 
      ...
      "</EndpointReference>";
    Element eprElem = XmlUtil.parseElement(text);
  ...
  public void testWriteEndpointRef() {
    ...
    Element addressElem = XmlUtil.getElement(eprElem, WsaConstants.NS_ADDRESSING, WsaConstants.ELEM_ADDRESS);
    assertEquals(address, XmlUtil.getStringValue(addressElem));
    
    Element portTypeElem = XmlUtil.getElement(eprElem, WsaConstants.NS_ADDRESSING, WsaConstants.ELEM_PORT_TYPE);
    String portTypePrefixedName = (String) XmlUtil.getStringValue(portTypeElem);
    assertEquals(portTypeName, XmlUtil.getQName(portTypePrefixedName, portTypeElem));
    
    Element serviceElem = XmlUtil.getElement(eprElem, WsaConstants.NS_ADDRESSING, WsaConstants.ELEM_SERVICE_NAME);
    String servicePrefixedName = (String) XmlUtil.getStringValue(serviceElem);
    assertEquals(serviceName, XmlUtil.getQName(servicePrefixedName, serviceElem));
