...
public class WsdlEndpointReferenceTest extends SoapEndpointReferenceTest {

  public WsdlEndpointReferenceTest(String name) {
    super(name);
  }
  ...
  public void testReadEndpointRef() throws Exception {
    String text = 
      "<wsdl:service name='pizzaService' xmlns='urn:pizzas:srv'" +
  ...
  public void testWriteEndpointRef() {
    ...
    assertEquals(serviceName.getLocalPart(), eprElem.getAttribute(Constants.ATTR_NAME));
    assertEquals(serviceName.getNamespaceURI(), eprElem.getAttribute("xmlns"));
    
    Element portElem = XmlUtil.getElement(eprElem, Constants.NS_URI_WSDL, Constants.ELEM_PORT);
    assertEquals(portName, portElem.getAttribute(Constants.ATTR_NAME));
    
    Element addressElem = XmlUtil.getElement(portElem, SOAPConstants.NS_URI_SOAP, SOAPConstants.ELEM_ADDRESS);
