...
public class XmlUtilTest extends TestCase {
  ...
  public void testRemoveChildNodes_dom() throws Exception {
    String xml =
      "<lunch xmlns:produce='urn:example:produce' xmlns='urn:example:meal'>" +
      " <time>1200</time>" +
  ...
  public void testRemoveChildNodes_soap() throws Exception {
    ...
    ByteArrayInputStream sourceStream = new ByteArrayInputStream(xml.getBytes());
    SOAPMessage soapMessage = MessageFactory.newInstance().createMessage(null, sourceStream);    
    SOAPElement element = XmlUtil.getElement(soapMessage.getSOAPBody(), "urn:example:meal", "lunch");
    // remove the child nodes
  ...
  public void testCopyNamespaces_domDom() throws Exception {
    String xml = 
      "<part xmlns:produce='urn:example:produce'>" +
      " <lunch produce:lettuce='0.1lb' fish:fillet='0.25lb' " +
      "  xmlns:fish='urn:example:fish' xmlns='urn:example:meal'/>" +
      "</part>";
    Element source = XmlUtil.getElement(XmlUtil.parseElement(xml), "urn:example:meal", "lunch");
    Element target = XmlUtil.createElement(null, "detail");
  ...
  public void testCopyVisibleNamespaces_domDom_targetMatch() throws Exception {
    ...
    String targetXml = 
      "<detail xmlns:produce='urn:example:produce' xmlns='urn:example:meal'>" +
      " <other:target xmlns:other='urn:example:other'/>" +
      "</detail>";
    Element target = XmlUtil.getElement(XmlUtil.parseElement(targetXml), "urn:example:other", "target");
  ...
  public void testCopyChildNodes_domDom() throws Exception {
    String xml =
      "<lunch xmlns:produce='urn:example:produce'" +
  ...
  public void testCopyAttributes_soapDom() throws Exception {
    ...
    XmlUtil.copyAttributes(target, source);
    // qualified attributes
    assertEquals("0.1lb", target.getAttributeValue(soapFactory.createName("lettuce", null, "urn:example:produce")));
    assertEquals("0.25lb", target.getAttributeValue(soapFactory.createName("fillet", null, "urn:example:fish")));
    // local attribute
  ...
  public void testCopyNamespaces_soapDom() throws Exception {
    ...
    assertEquals("urn:example:meal", target.getNamespaceURI(XmlUtil.DEFAULT_NAMESPACE_PREFIX));
  }
  ...
  public void testCopyVisibleNamespaces_soapDom_targetMatch() throws Exception {
    ...
    SOAPElement operation = XmlUtil.getElement(soapMessage.getSOAPBody(), "urn:example:other", "Operation");
  ...
  public void testCopyChildNodes_soapDom() throws Exception {
    ...
    // qualified, prefixed element
    SOAPElement lettuce = XmlUtil.getElement(target, "urn:example:produce", "lettuce");
    assertEquals("produce", lettuce.getPrefix());
    // heir qualified, prefixed element
    SOAPElement fillet = XmlUtil.getElement(target, "urn:example:fish", "fillet");
    assertEquals("fish", fillet.getPrefix());
    // local element
  ...
  public void testCopyNamespaces_domSoap() throws Exception {
    String xml = 
      ...
      "</soap:Envelope>";
    SOAPMessage soapMessage = toSOAPMessage(xml);    
    SOAPElement source = XmlUtil.getElement(soapMessage.getSOAPBody(), "urn:example:meal", "lunch");
    Element target = XmlUtil.createElement(null, "detail");
  ...
  public void testCopyVisibleNamespaces_domSoap_targetMatch() throws Exception {
    ...
    String targetXml = 
      "<detail xmlns:produce='urn:example:produce'>" +
      " <other:target xmlns:other='urn:example:other'/>" +
      "</detail>";
    Element target = XmlUtil.getElement(XmlUtil.parseElement(targetXml), "urn:example:other", "target");
  ...
  // BPEL-124: child elements in the default namespace are incorrectly losing their namespaces
  public void testCopy_soapDom_qualifiedNoPrefix() throws Exception {
    ...
    assertEquals("http://my.namespace", target.getNamespaceURI(XmlUtil.DEFAULT_NAMESPACE_PREFIX));
    // qualified elements
    SOAPElement firstName = XmlUtil.getElement(target, "http://my.namespace", "firstName");
    assertEquals("Martin", firstName.getValue());
    SOAPElement secondName = XmlUtil.getElement(target, "http://my.namespace", "secondName");
  ...
  public void testCopy_soapDom_noOverride() throws Exception {
    ...
    /* here, note the 'urn:example:meal' namespace, the default namespace in the source,
     * is mapped to prefix 'fish' which the source maps to namespace 'urn:example:fish' */ 
    String targetXml =
      "<soap:Envelope xmlns:soap='" + SOAPConstants.URI_NS_SOAP_ENVELOPE + "'>" +
      " <soap:Body>" +
      ...
      "</soap:Envelope>";
    SOAPMessage soapMessage = toSOAPMessage(targetXml);
    SOAPElement operation = XmlUtil.getElement(soapMessage.getSOAPBody(), "urn:example:meal", "Operation");
    ...
    soapMessage = MessageFactory.newInstance().createMessage(null, new ByteArrayInputStream(soapSink.toByteArray()));
    operation = XmlUtil.getElement(soapMessage.getSOAPBody(), "urn:example:meal", "Operation");
    ...
    // default declaration (reassigned)
    assertEquals("urn:example:meal", lunch.getNamespaceURI(XmlUtil.DEFAULT_NAMESPACE_PREFIX));
  }
  ...
  private static SOAPMessage toSOAPMessage(String xmlString) throws IOException, SOAPException {
