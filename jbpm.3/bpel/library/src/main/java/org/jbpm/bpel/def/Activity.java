...
public abstract class Activity extends Node {
  ...
  public void linkDetermined(Token targetToken) {
    ...
    if (!linksDetermined(targetToken)) return;
  ...
  public void leave(ExecutionContext executionContext) {
    ...
    if (sources != null) {
      for (Iterator it = sources.iterator(); it.hasNext();) {
        ...
        try {
          ...
          // cancelation
          if (executionContext.getToken().hasEnded()) return;
        }
    ...
    if (sourcesFault != null) throw sourcesFault;
  ...
  protected boolean continueExecution(Token token) {
    ...
    if (evaluateJoinCondition(token)) return true;
  ...
  private boolean evaluateJoinCondition(Token token) {
    ...
    while (targetIt.hasNext()) {
      ...
      if (linkInstance.getStatus() == Boolean.TRUE) return true;
    }
    return false;
  }
  ...
  public LinkDefinition getTarget(String linkName) {
    if (targets != null) {
      for (Iterator it = targets.iterator(); it.hasNext();) {
        ...
        if (linkName.equals(aTarget.getName())) return aTarget;
      }
    }
    return null;
  }
  ...
  public LinkDefinition getSource(String linkName) {
    for (Iterator it = sources.iterator(); it.hasNext();) {
      LinkDefinition aSource = (LinkDefinition) it.next();
      if (linkName.equals(aSource.getName())) return aSource;
    }
    return null;
  }
  ...
  /**
   * Connects a node to another one by creating an implcit transition between
   * them.
   */
  ...
  public boolean isConnected(Node to) {
    if (leavingTransitions != null && to.getArrivingTransitions() != null) {
      for (Iterator it = to.getArrivingTransitions().iterator(); it.hasNext();)
        if (leavingTransitions.contains(it.next())) return true;
    }
