...
public class DeploymentForm implements Observer {
	...
	public DeploymentForm(FormToolkit toolkit, Composite composite, IFolder processFolder, DesignerEditor editor) {
		this.toolkit = toolkit;
		this.composite = composite;
		this.processFolder = processFolder;
		this.editor = editor;
		editor.getProcessDefinition().addObserverOfClassNameChanges(this);
	}	
	...
	private void createServerNameField(Composite infoFormClient) {
		...
		String nameString = DesignerPlugin.getDefault().getPreferenceStore().getString("server name");
	...
	private void createServerPortField(Composite infoFormClient) {
		...
		String portString = DesignerPlugin.getDefault().getPreferenceStore().getString("server port");
	...
	private void createServerDeployerField(Composite infoFormClient) {
		...
		String deployerString = DesignerPlugin.getDefault().getPreferenceStore().getString("server deployer");
	...
	private Set getElementsToCheckFor(IJavaProject project) {
		...
		try {
			Set javaClassNames = editor.getProcessDefinition().getReferencedJavaClassNames();
		...
		for (int i = 0; i < objectsToRefresh.size(); i++) {
			...
			if (javaElement != null && javaElement instanceof ICompilationUnit) {
				if (referencedJavaClassNames == null) {
					referencedJavaClassNames = editor.getProcessDefinition().getReferencedJavaClassNames();
				}
	...
	public void update(Observable o, Object arg){
		if (!(o instanceof ProcessDefinition.ClassNameChangedRouter)) return;
		final IJavaProject project = JavaCore.create(processFolder.getProject());
		form.getDisplay().asyncExec(new Runnable() {
			public void run() {
				includeClassesTreeViewer.setCheckedElements(getElementsToCheckFor(project).toArray());
