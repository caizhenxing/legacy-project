...
definedWin.openWindow = function(txtName,winUrl,indexNum) {
...
definedWin.openModalWindow = function(txtName,url){
	definedWin.txtName = txtName;
	definedWin.setSelects("hidden");
	if(top != self){
		top.definedWin.openModalWindow(txtName,url);
...
definedWin.openListingUrl = function(txtName,url,no){
	no = no ? no : 0 ;
	alert(top.topFrame.definedWin);
	top.definedWin.oldSelectListing = top.definedWin.selectListing;
	top.definedWin.oldReturnArray_0 = top.definedWin.returnArray[0];
	definedWin.txtName = txtName;
	definedWin.setSelects("hidden");
	if(top != self){
		top.definedWin.openListingUrl(txtName,url,no);
...
definedWin.openLongTextWin = function(txtObj,hidObj,readonly,maxlength){
	definedWin.setSelects("hidden");
	...
	var hei = definedWin.height - definedWin.buttonHeight;
...
definedWin.show = function(no){
	...
	objContent.style.height = definedWin.height + 'px';
	objContent.style.width  = definedWin.width  + 'px';
	objLightbox.childNodes[0].childNodes[0].width = definedWin.width - 4;
	objLightbox.style.display = 'block';	
	var ChangeSpace =  20;
...
definedWin.setSelects = function(flag){
