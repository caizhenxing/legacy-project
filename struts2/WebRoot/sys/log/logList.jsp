...
<html>
	<body>
		...
		CurrentPage.query = function() {
			...
			FormUtils.post(document.forms[0], '<c:url value="/sys/log/list.do"/>');
		}
		CurrentPage.selectUser = function(assetInfoPre, definedWin, iCheckMode){
		 
		var url = ContextInfo.contextPath + "/sys/user/list.do?entityClass=com.cc.sys.db.SysUser&pageId=sys_user&checkType=" + iCheckMode;
		definedWin.openListingUrl(assetInfoPre, url);
	}
		...
		<input type="hidden" name="entityClass" value="com.cc.sys.db.SysLog"/>
			<jsp:directive.include file="/sys/log/logSearch.jspf"/>
