...
import base.zyf.web.crud.service.ViewBean.ViewAssi;
...
"CurrentPage.onLoadSelect();\r\n"+
	...
	public int doEndTag() throws JspException {
		...
		{
			popSelectType = (String)ExpressionEvaluationUtils.evaluate("formName","${popSelectType}", pageContext);
		...
		try {
				...
				out.print("<input  width='15px' id='detailIdsForPrintAll' type='checkbox' onclick=\"FormUtils.checkAll(this,document.getElementsByName('oid'))\" title=\"是否全选\"/>");
					...
					out.print("&nbsp;");
				out.print("</td>");
				out.println("<td  width=\"40px\" nowrap=\"nowrap\" type=\"operate\">序号</td>");
				while(rli.hasNext())
				{
		...
		int sp = 1;
		int showPage = ((PageInfo)ExpressionEvaluationUtils.evaluate("formName","${pageInfo}", pageContext)).getShowPage() - 1;
...
//				}
			...
			if (!"false".equals(this.del)) {
					out.print("<td nowrap='nowrap' align=\"center\">");
					out.print("<input type=\"button\" class=\"list_delete\" onclick=\"CurrentPage.remove('"
									+ id + "')\" title=\"点击删除\"/>");
					out.print("</td>");
				}
			out.println("</tr>");
