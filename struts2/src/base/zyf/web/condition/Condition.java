 ...
 * @author scott
 * @since 2006-4-18
 * @version $Id: Condition.java,v 1.1 2007/11/05 03:16:03 yushn Exp $
...
public class Condition implements ConditionConstants, Serializable, Comparable {
    ...
    public void setOperator(String operator) {
    	...
    	{
    		this.operator = ConditionConstants.LESS_EQUAL;
    	}else
    	if("ge".equals(operator))
    	{
            this.operator = ConditionConstants.GREATER_EQUAL;
    	}else
    	{
    ...
    public void setValue(Object value) {
    	...
    	}else
    	{
    		if(Date.class.equals(this.type))
    		{
    			...
    			if (String[].class.isInstance(value)) {
    				time = ((String[]) value)[0].trim();
    			}
    			if(StringUtils.isNotBlank(time))
    			{
    				if(time.length() == 10)
    					this.value = Tools.getTheTime(time, "yyyy-MM-dd");
    				if(time.length() == 19)
    				{
    					this.value = Tools.getTheTime(time, "yyyy-MM-dd HH:mm:ss");
    				}
    			}
    		}
