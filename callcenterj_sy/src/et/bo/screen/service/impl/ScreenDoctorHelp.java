...
//		}
		...
		hql.append("select oma from ScreenDoctor oma where oma.id = oma.id");
...
//		mq.setDetachedCriteria(dc);
	...
	public MyQuery marketInfoQuery(int size,String type) {
		MyQuery mq = new MyQueryImpl();
//		DetachedCriteria dc = DetachedCriteria.forClass(OperMarketAnalysis.class);
//		
//		if(!"".equals(dto.get("typeTitle"))) {
...
//		}
		...
		StringBuffer hql = new StringBuffer();
		hql.append("select oma from ScreenDoctor oma where oma.id = oma.id and type= '"+type+"' ");
...
//		mq.setDetachedCriteria(dc);
	...
	public MyQuery marketInfoQuery2() {
		...
		String hql = "from ScreenDoctor order by addTime desc";
		mq.setHql(hql);
		mq.setFirst(0);
		mq.setFetch(10);
		return mq;
