 ...
 * 
 */
public class MarkanaInfoByProductTypeImpl implements MarkanaInfoByProductTypeService {
	private BaseDAO dao;
	...
	public JFreeChart statistic(IBaseDTO dto) {
		...
		procSql.setProcedureName("proc_markanaInfoByProductType");
		...
		properties.put("chartTitle", "每一责任编辑各评别的产品数量");
		// 各种图形的特定属性
		String yChartName="每一责任编辑各评别的产品数量";
		if (chartType.indexOf(JFreeImpl.BAR) >= 0) {
			properties.put("xChartName", "日期");
		...
		if(xAxis.size()>0&&yAxis.size()>0) {
			for (Iterator<String> i = xAxis.iterator(); i.hasNext();) {
				...
				if(!"".equals(dto.get("name"))) {
					...
					r.set("count", cts.getLabelById(tmp));
					r.set("count1", valueList.get(xAxis.indexOf(tmp) * yAxis.size()));
				}
				else {
					r.set("name", tmp);
					r.set("count", cts.getLabelById(yAxis.get(j++)));  // 有问题？？
					r.set("count1", valueList.get(xAxis.indexOf(tmp) * yAxis.size()));
				}
