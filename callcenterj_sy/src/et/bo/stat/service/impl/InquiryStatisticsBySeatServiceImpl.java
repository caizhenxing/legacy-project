...
import et.bo.stat.service.InquiryStatisticsBySeatService;
 ...
 * 
 */
public class InquiryStatisticsBySeatServiceImpl implements
		InquiryStatisticsBySeatService {
	private BaseDAO dao;
	...
	public JFreeChart statistic(IBaseDTO dto) {
		...
		procSql.setProcedureName("proc_inquiryStatisticsBySeat");
		...
		properties.put("chartTitle", "座席员完成的问题与用户调查数量统计");
		// 各种图形的特定属性
		String xChartName = "".equals(dto.get("caseRid")) ? "受理工号" : "时间";
		String yChartName = "问题数量、用户数量";
	...
	public List<DynaBeanDTO> query(IBaseDTO dto) {
		...
		if (xAxis.size() > 0 && yAxis.size() > 0) {
			for (Iterator<String> i = xAxis.iterator(); i.hasNext();) {
				...
				for (int k = 0; k < yAxis.size(); k++) {
					...
					r.set("name", tmp);
					r.set("type1", valueList.get(xAxis.indexOf(tmp)
							* yAxis.size() + yAxis.indexOf(yAxis.get(k))));
					r.set("type2", valueList.get(xAxis.indexOf(tmp)
							* yAxis.size()));
					nums += Integer.valueOf(valueList.get(xAxis.indexOf(tmp)
							* yAxis.size()));
					r.set("nums1", nums);
					nums1 += Integer.valueOf(valueList.get(xAxis.indexOf(tmp)
							* yAxis.size() + yAxis.indexOf(yAxis.get(k))));
					r.set("nums2", nums);
					list.add(r);
				}
		...
		// r.set("nums", nums);
		// r.set("type2", valueList.get(xAxis.indexOf(tmp)
		// * yAxis.size() + yAxis.indexOf(processType)));
		// list.add(r);
		// }
		// }
		return list;
