/**
 * 	@(#) bookMedicinfoPServiceImpl.java 2008-4-14 下午01:09:59
 *	版权所有 沈阳市卓越科技有限公司。 
 *	卓越科技 保留一切权利
 */
 ...
 * 
 */
public class BookMedicinfoPServiceImpl implements BookMedicinfoPService {
	private BaseDAO dao;
	...
	public JFreeChart statistic(IBaseDTO dto) {
		...
		procSql.setProcedureName("proc_book_medicinfo");
		...
		if(xAxis.size()>0&&yAxis.size()>0)
		{
			for (Iterator<String> i = xAxis.iterator(); i.hasNext();) {
				...
				for (Iterator<String> j = yAxis.iterator(); j.hasNext();) {
					...
					// r.set("Xaxes", tmp);
					// r.set("Yaxes", valueList.get(xAxis.indexOf(tmp) *
					// yAxis.size()));
