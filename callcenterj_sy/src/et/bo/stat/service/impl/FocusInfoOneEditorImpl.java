 ...
 * 
 */
public class FocusInfoOneEditorImpl implements FocusInfoOneEditorService {
	private BaseDAO dao;
	...
	public JFreeChart statistic(IBaseDTO dto) {
		...
		procSql.setProcedureName("proc_focusInfoOneEditor");
		...
		properties.put("chartTitle", "每一责任编辑各版别的产品数量");
		// 各种图形的特定属性
		String yChartName="每一责任编辑各版别的产品数量";
	...
	public List<DynaBeanDTO> query(IBaseDTO dto) {
		...
		if(xAxis.size()>0&&yAxis.size()>0) {
			for (Iterator<String> i = xAxis.iterator(); i.hasNext();) {
				...
				r.set("date", tmp);  //日期
				r.set("count", valueList.get(xAxis.indexOf(tmp) * yAxis.size()+1));  //版别
				r.set("count1", valueList.get(xAxis.indexOf(tmp) * yAxis.size()));  //数量
	...
	private List<String> getValues(List result, List<String> xAxis,
		...
		for (Iterator i = ((List) result.get(2)).iterator(); i.hasNext();) {
			...
			if(record.get("xaxis")!=null && record.get("yaxis") != null && record.get("value") != null){
				String respondent = record.get("xaxis").toString();
				String processType = record.get("yaxis").toString();
				String value = record.get("value").toString();
				String yaxis=record.get("yaxis").toString();
				valueList.set((xAxis.indexOf(respondent)) * yAxis.size(), value);
				valueList.set((xAxis.indexOf(respondent)) * yAxis.size()+1, yaxis);
			}
