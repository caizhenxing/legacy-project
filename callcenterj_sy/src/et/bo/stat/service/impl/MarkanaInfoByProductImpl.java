 ...
 * 
 */
public class MarkanaInfoByProductImpl implements MarkanaInfoByProductService {
	private BaseDAO dao;
	...
	public JFreeChart statistic(IBaseDTO dto) {
		...
		procSql.setProcedureName("proc_markanaInfoByProduct");
		...
		properties.put("chartTitle", "全部品种和各品种的产品数量");
		// 各种图形的特定属性
		String yChartName="全部品种和各品种的产品数量";
		if (chartType.indexOf(JFreeImpl.BAR) >= 0) {
			properties.put("xChartName", "品种");
	...
	public List<DynaBeanDTO> query(IBaseDTO dto) {
		...
		// 调用存储过程取得统计结果
		StatDateStr.setBeginEndTime(dto);
//		String dictProductType1=dto.get("dictProductType1").toString();
//		String dictProductType2=dto.get("dictProductType2").toString();
//		String sql="";
//		if(!dictProductType1.equals("")){
//			sql+=" and dictProductType1='"+dictProductType1+"' ";
//		}
//		if(!dictProductType2.equals("")){
//			sql+=" and dictProductType2='"+dictProductType2+"' ";
//		}
		List<String> params = new ArrayList<String>();
		List results = new ArrayList();
		params.add(dto.get("beginTime").toString());
		...
		for(int i=0; i<lv.size(); i++)
		{
			...
			try
			{
				int i_1 = Integer.parseInt(m.get("年评").toString());
				int i_2 = Integer.parseInt(m.get("季评").toString());
				int i_3 = Integer.parseInt(m.get("月评").toString());
				int i_4 = Integer.parseInt(m.get("周评").toString());
