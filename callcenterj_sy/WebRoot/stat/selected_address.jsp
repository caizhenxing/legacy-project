...
<%
			...
			//System.out.println("ContextLoaderPlugIn.SERVLET_CONTEXT_PREFIX="+ContextLoaderPlugIn.SERVLET_CONTEXT_PREFIX);
			//System.out.println(application.getAttribute("org.springframework.web.struts.ContextLoaderPlugIn.CONTEXT."));
	BasicDataSource bds = (BasicDataSource) ac.getBean("datasource");//获得数据源
	...
	if (sid.equals("") && svalue.equals(""))
	{
		try
		{
			...
			rs = stmt.executeQuery("select distinct city from oper_address order by city");
			while(rs.next())
			{
...
<html>
	...
	<script>
		function select1()
		{
			...
			<%--fixedContact是ActionForm的属性名 --%>
			opener.document.forms[0].address.value = city + section + villages + community;
			window.close();
		}
		var changeid = "";
		function select2(obj)
		{
			...
			if(svalue != "" && sid != "community"){
				sendRequest("selected_address.jsp", "sid="+sid+"&svalue="+svalue, processResponse);
			}
		}
...
<%
	...
	else if (!sid.equals("") && !svalue.equals(""))
	{
		...
		String sql = "select distinct " + sid + " from oper_address where " + whereid + " = '"
		+ svalue + "' order by " + sid;
		try
		{
			conn = bds.getConnection();
		...
		str += "<option value='" + s + "'>" + s + "</option>";
			...
			out.print("<select name='" + selectid + "' id='" + selectid
			+ "' onChange='select2(this)' class='selectStyle'><option>请选择</option>" + str
			+ "</select>");
		}
		catch(Exception e)
		{
