...
function writeIframe()
{
    ...
    "case 84 : document.forms[0].today.click(); break; case 67 : parent.setVal(1) ;parent.hiddenCalendar();parent.setVal(0) ;break;} window.event.keyCode = 0; window.event.returnValue= false;}"+
    ...
    "    <table id=tableDay height=120 width=140 border=0 cellspacing=1 cellpadding=0>";
         ...
         " type=button value='关闭' onclick='parent.hiddenCalendar()'></td>"
         strIframe +="<td colspan=2 class=out title='清空选中的日期&#13;快捷键：C'><input style=' background-color: "+
         WebCalendar.btnBgColor +";cursor: hand; padding-top: 4px; width: 100%; height: 100%; border: 0' onfocus='this.blur()'"+
         " type=button value='清空' onclick='parent.setVal(1) ;parent.hiddenCalendar();parent.setVal(0) ;'></td>"+
         "</tr></table>"+
...
function WebCalendar() //初始化日历的设置
{
    ...
    this.clearInputVal = 0        //点击“关闭”按钮时是否清空文本框内容(1-清空,0-不清空)

    this.yearFall   = 50;           //定义年下拉框的年差值
    this.format     = "yyyy-mm-dd"; //回传日期的格式
    this.timeShow   = false;        //是否返回时间
    this.drag       = true;         //是否允许拖动
    this.darkColor  = "#5BA2D8";    //控件的暗色
...
function setVal(val){WebCalendar.clearInputVal=val;}
function hiddenCalendar(){ if(WebCalendar.clearInputVal==1) WebCalendar.objExport.value=""; getObjectById("meizzCalendarLayer").style.display = "none";};
