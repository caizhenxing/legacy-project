   	...
   	<script type="text/javascript">	
   	    ...
   	    function removeLast(obj)
   	    {
   	    	...
   	    	if(value!='')
   	    	{
   	    		...
   	    		if(l==value.length-1)
   	    		{
   	    			obj.value = obj.value.substring(0,(value.length-1));
   	    		}
   	    	}
   	    }
...
<%--			$("#custName").formValidator({onshow:"请输入用户姓名",onfocus:"用户姓名不能为空",oncorrect:"用户姓名合法"}).inputValidator({min:1,empty:{leftempty:false,rightempty:false,emptyerror:"用户姓名两边不能有空符号"},onerror:"用户姓名不能为空"});--%>
			$("#custAddr").formValidator({onshow:"请输入地址",onfocus:"地址不能为空",oncorrect:"地址合法"}).inputValidator({min:1,empty:{leftempty:false,rightempty:false,emptyerror:"地址两边不能有空符号"},onerror:"地址不能为空"});
		})		
	</script>	
...
<%--onsubmit="return checkForm(this)"--%>
	<html:form action="operpriceinfo.do?method=toOperPriceinfo&type=insertList" method="post" styleId="PriceInfo">
			...
			<tr>
				...
				<td class="valueStyle" colspan="2">
					...
					<img alt="选择时间" src="../html/img/cal.gif"
						onclick="openCal('priceinfoBean','deployTime',false);">
...
<%--					<span id="custNameTip" style="width: 10px;display:inline;"></span>--%>
			...
			<tr>
				...
				<td class="labelStyle">
					<input name="product_price" type="text" class="writeTextStyle" id="product_price" size="10"  onkeyup="numValidate(this)" onblur="removeLast(this)" />
				</td>
				<td class="labelStyle">
...
<script>
	...
	function addtr(){
		...
		td = tr.insertCell();
		td.className = "labelStyle";
		td.innerHTML = '<input name="product_price" type="text" class="writeTextStyle" id="product_price" size="10" onkeyup="numValidate(this)" onblur="removeLast(this)">';
		td = tr.insertCell();
		td.className = "labelStyle";
