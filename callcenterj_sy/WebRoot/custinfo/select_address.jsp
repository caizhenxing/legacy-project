	...
	<body class="loadBody">
		<form name="address" method="post" action="">
			<table width="450" border="0" align="center">
				...
				<tr>
					<td  class="valueStyle">
						<select name="city" id="city" onChange="select2(this)" class="selectStyle">
							<option value="">
								请选择市
							</option>
							<%= city %>
...
<script>
	...
	function select1(){
		...
		opener.document.forms[0].cust_addr.value = city + section + villages + community;
		window.close();
	}
...
<%
		...
		String sql = "select distinct "+ sid +" from oper_address where "+ whereid +" = '"+svalue+"' order by "+ sid;
		try {
			conn = bds.getConnection();
			stmt = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
			...
			out.print("<select name='"+ selectid +"' id='"+ selectid +"' onChange='select2(this)' class='selectStyle'><option>请选择</option>"+str+"</select>");
		} catch (Exception e) {
			System.err.println(e);
		} finally {
			try {
