...
<%
	...
	if(table !=null && !table.equals("") && id !=null && !id.equals("") && index !=null && !index.equals("")){
		...
		try {
			...
			rs = stmt.executeQuery("select cust_pic_path from "+ table +" where "+ field +" = '" + id + "'");
			if(rs.next()){
				...
				for(int i = 0; i<imgs.length; i++){
					if(i == j){										//如果是要删除的文件，则删除操作
						...
						if(file.exists()){
							file.delete();	//删除文件
							stmt.executeUpdate("update "+table+" set cust_pic_path = '' where cust_id = '"+id+"'");
						}
					}else{
				...
				int i = stmt.executeUpdate("update "+ table +" set cust_pic_path = '"+ newUploadFile +"' where "+ field +" = '" + id + "'");
				if(i > 0){
					out.print("<script>");
