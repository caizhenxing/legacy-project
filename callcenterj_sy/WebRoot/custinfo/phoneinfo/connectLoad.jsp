...
<%--				$("#cust_age").formValidator({empty:true,onshow:"请输入年龄，可以为空",onfocus:"如果输入，必须输入正确",oncorrect:"年龄",onempty:"没有填写年龄"}).inputValidator({min:1,max:99,type:"value",onerrormin:"年龄必须大于等于1",onerrormax:"年龄必须小于等于99",onerror:"年龄必须在1-99之间"});	
				$("#dict_cust_voc").formValidator({onshow:"请选择从事行业",onfocus:"从事行业必须选择",oncorrect:"OK!"}).inputValidator({min:1,onerror: "没有选择从事行业"});
				...
				$("#cust_develop_time").formValidator({onshow:"请输入入队时间",onfocus:"入队时间不能为空",oncorrect:"入队时间合法"}).inputValidator({min:1,empty:{leftempty:false,rightempty:false,emptyerror:"入队时间两边不能有空符号"},onerror:"入队时间不能为空"});--%>
			})
			</logic:equal>
			<logic:equal name="opertype" value="update">
			$(document).ready(function(){
