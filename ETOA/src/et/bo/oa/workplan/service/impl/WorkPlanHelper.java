/**
 * 	@(#)WorkPlanHelper.java   2006-11-13 下午01:28:42
 *	 。 
 *	 
 */
...
//		dc.add(Restrictions.eq("planmissionSignDel", "0"));
		if(ac.getMissionselectsign().equals("on"))
		{
			
			dc.add(Restrictions.eq("planmissionSignDel", "1"));
	...
	//	public MyQuery plan(String user,String title,String subhead,String begin,String end,String begin1,String end1,PageInfo pi,String checkId,String auditingtype)
	
	public MyQuery plan(String user,String title,String subhead,String begin,String end,String begin1,String end1,PageInfo pi,String checkId,String planTimeType)
	{
		
		//auditingtype是plan页面传过来的审核类型
		MyQuery mq=new MyQueryImpl();
		...
		//类型审核的查询
		if(checkId.equals(ac.getAuditingNo()))
		{
			dc.add(Restrictions.like("checkId","%"+checkId+"%"));
		}

		if(checkId.equals(ac.getAuditingYes()))
		{
			dc.add(Restrictions.like("checkId","%"+checkId+"%"));
		}		
		if(checkId.equals(ac.getAuditingNot()))
		{
			dc.add(Restrictions.like("checkId","%"+checkId+"%"));
		}

		if(checkId.equals(ac.getAuditingDel()))
		{
		...
		//时间种类的查询
		if(planTimeType.equals(ac.getPlanTimeWeek()))
		{
			dc.add(Restrictions.like("planTimeType","%"+planTimeType+"%"));
		}
		
		if(planTimeType.equals(ac.getPlanTimeMother()))
		{
			dc.add(Restrictions.like("planTimeType","%"+planTimeType+"%"));
		}
		
		if(planTimeType.equals(ac.getPlanTimePhase()))
		{
			dc.add(Restrictions.like("planTimeType","%"+planTimeType+"%"));
		}
		...
		{
			dc.add(Restrictions.eq("delSign","0"));
			
		}
		
		dc.add(Restrictions.eq("planType","1"));
		...
		MyQuery mq=new MyQueryImpl();
			...
			dc.add(Restrictions.eq("checkId",ac.getAuditingYes()));
		...
		public MyQuery myMission1(UserInfo ui)
		{
			MyQuery mq=new MyQueryImpl();
		...
		public MyQuery mymyMission(UserInfo ui)
		{
			MyQuery mq=new MyQueryImpl();
		...
		public MyQuery mymymyMission(UserInfo ui)
		{
			MyQuery mq=new MyQueryImpl();
		...
		public MyQuery mymissionpage(String user,String title,String subhead,String begin,String end,String begin1,String end1,PageInfo pi,String checkId,String planTimeType)
		{
