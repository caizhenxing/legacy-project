...
<html:html locale="true">
  <head>
    ...
    <script language="javascript">
		...
		//还原指定行
		function revLine(articleid){
				document.forms[0].action = "../opernews.do?method=operRecycleInfo&oper=rev&id="+articleid;
				document.forms[0].submit();	
		}
    </script>
  </head>
    ...
    <html:form action="/news/opernews.do" method="post">
		...
		<table width="80%" border="0" align="center" cellpadding="1" cellspacing="1" class="tablebgcolor">
		  <tr class="tdbgpiclist">
		    <td><bean:message key="agrofront.news.article.recyclelist.select"/></td>
		    <td><bean:message key="agrofront.news.article.recyclelist.title"/></td>
		    <td><bean:message key="agrofront.news.article.recyclelist.author"/></td>
		    <td><bean:message key="agrofront.news.article.recyclelist.updatetime"/></td>
		    <td><bean:message key="agrofront.news.article.recyclelist.state"/></td>
		  ...
		  <tr>
		    ...
		    <img alt="<bean:message key='agrofront.news.article.recyclelist.recove'/>" src="<bean:write name='imagesinsession'/>sysoper/rev.gif" onclick="revLine('<bean:write name='c' property='articleid'/>')" width="16" height="16" border="0"/>
		    <img alt="<bean:message key='agrofront.news.article.recyclelist.delete'/>" src="<bean:write name='imagesinsession'/>sysoper/delforever.gif" onclick="delIt('<bean:write name='c' property='articleid' filter='true'/>')" width="16" height="16" border="0"/>	      	    
		    </td>
		  </tr>
		  </logic:iterate>
		  <tr>
